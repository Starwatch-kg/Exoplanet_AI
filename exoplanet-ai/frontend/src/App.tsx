...
const CatalogPage = lazy(() => import('./pages/CatalogPageNew'));
const BLSPage = lazy(() => import('./pages/BLSPage'));
...
const App: React.FC = () => {
  ...
  const [useRouting, setUseRouting] = useState(true); // Переключатель между режимами
  ...
  return (
    <AppContext.Provider value={contextValue}>
      <div className={`min-h-screen text-white transition-all duration-300 ${theme === 'light' ? 'bg-gray-50 text-gray-900' : 'bg-space-gradient'}`}>
        <StarBackground />
        
        {useRouting ? (
          // Режим с маршрутизацией
          <>
            <Navbar />
            <motion.main
              initial={{ opacity: 0 }}
          ...
          // Режим без маршрутизации (single page)
          <>
            <Header />
