...
async def pro_analysis(request: ProAnalysisRequest):
    ...
    try:
        ...
        if request.model_type == "detector":
            try:
                from ml_models.exoplanet_detector import ExoplanetEnsembleDetector as DetectorModel
                model_class = DetectorModel
            except ImportError:
                from ml_models.models import ExoplanetDetector as DetectorModel
                model_class = DetectorModel
        elif request.model_type == "verifier":
            from ml_models.exoplanet_detector import LSTMTimeSeriesAnalyzer as VerifierModel
            model_class = VerifierModel
        elif request.model_type == "pro":
            from ml_models.exoplanet_detector import AutoencoderAnomalyDetector as ProModel
        ...
        if request.model_type == "detector":
            model = model_class(input_shape, num_classes=3)
        elif request.model_type == "verifier":
            model = model_class(input_shape, num_classes=3)
        elif request.model_type == "pro":
            model = model_class(input_shape, latent_dim=32)

        # Build model if needed
        if hasattr(model, 'build_models'):
            model.build_models()
        ...
        if request.model_type == "detector":
            ...
            predictions, errors = model.predict_anomaly(X.reshape(1, -1, 1))
            predictions = model.predict_proba(X.reshape(1, -1, 1))
